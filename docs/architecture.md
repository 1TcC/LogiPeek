...
- `app::window` owns one fixed 400 x 640 logical-pixel, Per-Monitor-V2-aware Win32 settings window. It custom-draws the first-run language picker, cards, device selector, battery progress, slider, presets, appearance/language choices, startup/alert controls, and inline status into a per-paint memory back buffer, then presents the complete client with one `BitBlt`. Hover invalidates only when the hit target changes, and `TrackMouseEvent` clears hover on `WM_MOUSELEAVE`. Closing hides it; tray Open shows the same HWND.
- `app::state` is the single hardware and UI truth consumed by tray and window. It binds device identity, battery, and DPI to one selected endpoint. One writable target is selected automatically; multiple targets require a unique remembered or explicit selection before controls are enabled.
...
- `app::startup` contains mockable policy for one current-user Run value. Raw registry calls remain in the binary-only tray module. `--startup` creates the normal tray process but leaves the main window hidden.
- `app::battery_alert` tracks threshold crossings and a five-percent rearm hysteresis. The tray presents localized `Shell_NotifyIconW` balloons only for exact percentages while discharging.
...
The tray menu and settings window render locked clones of the same `AppState`. Battery and DPI values therefore come from the same selected endpoint. Failed full scans replace them with unavailable state instead of presenting stale data as current. A battery-only refresh retains DPI only when the selected opaque identity is still present. Multiple usable targets without a verified selection disable every preset.

After startup, Refresh, recovery, or selection change, the worker retains an in-memory validated target. Each GUI/tray write reopens that exact HID identity, revalidates its slot, feature, sensor, and capability, then performs one function 3 attempt plus independent readback without probing unrelated offline slots. Target mismatch invalidates the cache before any write. The one-shot CLI keeps its complete unique-target preflight.
...
The runtime path is `%LOCALAPPDATA%\LogiPeek\settings.ini`. In addition to presets, theme, and language, it stores startup intent, notification enable/threshold, and an optional opaque selected-device ID. `language=None` remains the only onboarding state. Saving uses the existing synced temporary file and atomic replacement. The startup toggle alone manages the `LogiPeek` value in `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`; registry failure is reported and never presented as success. No database, serializer crate, update polling, or network access is involved.
...
Future work begins with broader multi-device hardware validation, evidence-backed receiver/Bluetooth interpretation, and incremental accessibility improvements. Profiles, `0x2202` writes, RGB, macros, remapping, model databases, and automatic updates remain outside this phase.
