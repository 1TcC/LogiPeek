...
Designed to support as many Logitech mice as practical through HID++ capability discovery. The 30 protocol tests, `cargo fmt --check`, `cargo check`, `cargo clippy --all-targets --all-features -- -D warnings`, and release build passed on 2026-09-19.
...
LogiPeek has no accounts, telemetry, analytics, uploads, cloud calls, background service, or polling loop. Requests are serial and bounded. After a write completes, the reply deadline is 750 ms and at most 128 incoming reports are processed. Explicit read-only operations may make one retry after Timeout or HID++ Busy; malformed data, I/O failures, and other protocol errors are returned immediately. A Windows backend write itself can take up to about one second, so 750 ms is not an entire-exchange limit. Diagnostics omit serial numbers and full HID paths.
