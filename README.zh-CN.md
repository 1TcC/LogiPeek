...
LogiPeek 没有账户、遥测、分析、上传、云端调用、后台服务或轮询循环。请求串行且有上限。一次写入完成后，回复截止时间为 350 ms，最多处理 128 个入站报文。Windows backend 的单次写入本身最多可能耗时约一秒，因此 350 ms 不是整个 exchange 的上限。诊断输出省略序列号和完整 HID 路径。
