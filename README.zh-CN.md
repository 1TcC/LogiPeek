...
无参数启动时，LogiPeek 运行原生 Windows 通知区域程序并打开紧凑设置窗口；带 CLI 参数时仍执行一次操作后退出。GUI、托盘和 CLI 在新的安全预检后复用同一条 `0x2201` function 3 写入路径。已于 2026-09-19 在 Windows 上实测一个接收器配置；详见 [实机观察](docs/hidpp.md#baseline-hardware-observation)。它不承诺支持具体鼠标型号或广泛的接收器兼容性，转发槽位可达性仍有间歇性。
...
- 后续可继续改进原生窗口无障碍支持和硬件覆盖。设备 profile、`0x2202` 写入、RGB、宏、重映射、后台服务、开机启动、通知、设备选择和更新仍不在当前阶段范围内。
