...
LogiPeek 是一个小型、非官方的 Windows 托盘与命令行工具，用于查看 Logitech HID++ 接口，并报告其公开的电池和 DPI 能力。它完全在本机运行。

## 动机

它面向希望以轻量方式查看鼠标电量和 DPI 能力、且不想安装完整设备套件的用户。它不是 Logitech G HUB 的完整替代品：没有配置文件、按键重映射、RGB、宏、主 GUI 窗口、后台服务或持久化配置。唯一的配置动作是经过验证的运行时 DPI 修改。

## 状态与目标

项目仍处于早期、以读取为主的基础阶段，并包含一个严格限定的写操作。无参数启动时，它运行原生 Windows 通知区域程序；带 CLI 参数时仍执行一次操作后退出。两种模式在新的安全预检后复用同一条 `0x2201` function 3 写入路径。已于 2026-09-19 在 Windows 上实测一个接收器配置；详见 [实机观察](docs/hidpp.md#baseline-hardware-observation)。它不承诺支持具体鼠标型号或广泛的接收器兼容性。该配置已成功完成真实 `0x1004` 电池和 `0x2201` DPI 读取，但转发槽位可达性仍有间歇性。

LogiPeek 通过 HID++ 能力探测机制，目标是尽可能兼容不同型号的 Logitech 鼠标。自动化测试包含 34 项协议测试和 5 项托盘状态测试。
...
无参数执行 `cargo run` 可启动托盘。固定 preset 不会保存为设置，所选 DPI 也只是设备的运行时值。
...
LogiPeek 没有账户、遥测、分析、上传、云端调用或后台服务。托盘使用 Win32 消息等待和一个阻塞 worker 队列，不做 busy polling；worker 每 60 秒刷新一次电池，并串行执行全部 HID 操作。请求发出后，回复截止时间为 750 ms，最多处理 128 个入站报文。明确的只读操作可在 Timeout 或 HID++ Busy 后重试一次；格式错误、I/O 错误和其他协议错误会立即返回。DPI setter 始终只进行一次物理写入尝试。诊断输出省略序列号和完整 HID 路径。
...
- 后续阶段可增加独立主窗口和用户自定义 preset。持久化、配置文件、`0x2202` 写入、RGB、宏、重映射、后台服务、开机启动和更新仍不在当前阶段范围内。
